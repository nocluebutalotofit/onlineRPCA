...
def pcp_v(M, lam=np.nan, mu=np.nan, factor=1, tol=10 ** (-7), maxit=1000):
    ...
    V : Eigenvectors, to transform use M_transformed = np.dot(M, V[0:rank,:].T)
    ...
    return L, S, niter, rank, V


def pcp(M, lam=np.nan, mu=np.nan, factor=1, tol=10 ** (-7), maxit=1000):
    L, S, niter, rank, _ = pcp_v(
        M, lam=np.nan, mu=np.nan, factor=1, tol=10 ** (-7), maxit=1000
    )
